...
export class EscrowService {
  ...
  async createEscrow(network: 'BEP20' | 'TRC20', gameId: string, amount: string) {
  ...
  async refundEscrow(network: 'BEP20' | 'TRC20', gameId: string, players: any[]) {
    // TODO: Refund escrow if game is cancelled
    this.logger.log(`Refunding escrow for cancelled game ${gameId}`);
    throw new Error('Method not implemented');
  }

  async getEscrowBalance(network: 'BEP20' | 'TRC20', gameId: string) {
    // TODO: Get current balance in escrow
    throw new Error('Method not implemented');
  }

  async verifyEscrowTransaction(network: 'BEP20' | 'TRC20', txHash: string) {
    if (network === 'BEP20') {
      return this.bscService.verifyTransaction(txHash);
    } else {
      return this.tronService.verifyTransaction(txHash);
    }
  }
}
