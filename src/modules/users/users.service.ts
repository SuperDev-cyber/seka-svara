...
export class UsersService {
  ...
  async findOne(id: string) {
    const user = await this.usersRepository.findOne({
      where: { id },
      select: ['id', 'username', 'email', 'role', 'status', 'avatar', 'createdAt', 'updatedAt'],
    });
  ...
  async generateWalletAddresses(userId: string) {
    ...
    const addresses = {
      TRC20: user.trc20WalletAddress || await this.walletService.generateDepositAddress(userId, 'TRC20'),
    ...
    await this.update(userId, {
      trc20WalletAddress: addresses.TRC20,
